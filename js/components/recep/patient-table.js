...
//         records = records.filter(r => r.status && r.status.toLowerCase() !== 'pending');

//         tbody.innerHTML = '';

//         if (records.length === 0) {
//             tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:3rem;color:#718096;">No confirmed or processed records found.</td></tr>`;
//             return;
//         }
...
window.updatePatientTable = async function () {
    ...
    try {
        ...
        // ✅ Filter out only pending records, keep all others including no-status
        records = records.filter(r => !r.status || r.status.toLowerCase() !== 'pending');
